...
from boards.platforms import ca23_platform
...
class BaseSoC(SoCCore):
    ...
    def __init__(self, board="ca23", sys_clk_freq=int(100e6), cpu_type="picorv32",
        with_bios             = False,
        with_uartbone         = False,
        ...
        # Platform ---------------------------------------------------------------------------------
        platform = ca23_platform.Platform()
        ...
        self.pcie_phy.update_config({
            ...
            "Vendor_ID"                : "10EE",
            "Device_ID"                : "7023",
            "Base_Class_Menu"          : "Multimedia device",
            "Sub_Class_Interface_Menu" : "Audio device",
            "Class_Code_Base"          : "04",
            "Class_Code_Sub"           : "01",
            "Revision_ID"              : "0001",
...
def main():
    ...
    parser = argparse.ArgumentParser(description="LiteX SoC on CA23.")

    parser.add_argument("--board",    default="ca23",         help="Select board.", choices=["ca23"])
    ...
    for run in range(2):
        ...
        if prepare:
            ...
            with open(env_mak, "w") as f:
                ...
                f.write("BSP_PROJECT_DIR=bsp/CA23\n")
            os.system(f"cd firmware && make clean all")
            bistream_output_dir = "bitstream/CA23"
    ...
    if args.flash:
        if args.gold:
            prog = soc.platform.create_programmer(cable=args.cable)
            prog.flash(0, os.path.join(bistream_output_dir, "CA23" + "_golden" + ".bin"), enable_quad=True)
        else: #user img
            # TODO: move user img address to a global variable somewhere instead of hardcoding
            prog = soc.platform.create_programmer(cable=args.cable)
            prog.flash(0X00220000, os.path.join(bistream_output_dir, "CA23" + "_user" + ".bin"), enable_quad=True)
