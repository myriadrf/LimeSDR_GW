...
from boards.platforms import limesdr_xtrx_platform, fairwaves_xtrx_platform, ssdr_platform
...
class BaseSoC(SoCCore):
    ...
    def __init__(self, board="ssdr", sys_clk_freq=int(100e6), cpu_type="picorv32",
        with_bios             = False,
        with_uartbone         = False,
        ...
        if with_bios:
            ...
            integrated_main_ram_size = 0x10000 if cpu_firmware is None else 0x6800
        ...
        self.comb += [ self.gpio.GPIO_DIR.eq(0b111011111),
                       self.gpio.GPIO_OUT_VAL[5].eq(self.limetop.lms7002_top.tx_ant_en)]

        self.comb += [
            self.limetop.rxtx_top.rx_path.pps.eq(self.gpio.GPIO_IN_VAL[6]),
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="LiteX SoC on SSDR.")

    parser.add_argument("--board",    default="ssdr",      help="Select XTRX board.", choices=["fairwaves_cs", "fairwaves_pro", "limesdr", "ssdr"])
    ...
    if args.flash:
        if args.gold:
            prog = soc.platform.create_programmer(cable=args.cable)
            prog.flash(0, os.path.join(bistream_output_dir, "SSDR" + "_golden" + ".bin"), enable_quad=True)
        else: #user img
            # TODO: move user img address to a global variable somewhere instead of hardcoding
            prog = soc.platform.create_programmer(cable=args.cable)
            prog.flash(0X00220000, os.path.join(bistream_output_dir, "SSDR" + "_user" + ".bin"), enable_quad=True)
    ...
    soc.generate_documentation(build_name + "_rev2", build_html=args.doc)
