...
//
// Filename   : fifo_w128x256_r64.v
// Device     : 
// LiteX sha1 : c1225736a
// Date       : 2024-11-07 10:12:05
//------------------------------------------------------------------------------
...
wire          cdc_cd_rst;
wire  [129:0] cdc_cdc_asyncfifo_din;
wire  [129:0] cdc_cdc_asyncfifo_dout;
...
wire  [127:0] cdc_cdc_fifo_in_payload_data;
wire          cdc_cdc_fifo_out_first;
wire          cdc_cdc_fifo_out_last;
wire  [127:0] cdc_cdc_fifo_out_payload_data;
...
reg    [63:0] converter_converter_source_payload_data = 64'd0;
...
assign converter_converter_first = (converter_converter_mux == 1'd0);
assign converter_converter_last = (converter_converter_mux == 1'd1);
assign converter_converter_source_valid = converter_converter_sink_valid;
assign converter_converter_source_first = (converter_converter_sink_first & converter_converter_first);
assign converter_converter_source_last = (converter_converter_sink_last & converter_converter_last);
assign converter_converter_sink_ready = (converter_converter_last & converter_converter_source_ready);
always @(*) begin
    converter_converter_source_payload_data <= 64'd0;
    case (converter_converter_mux)
        1'd0: begin
            converter_converter_source_payload_data <= converter_converter_sink_payload_data[63:0];
        end
        default: begin
            converter_converter_source_payload_data <= converter_converter_sink_payload_data[127:64];
        end
    endcase
end
assign converter_converter_source_payload_valid_token_count = converter_converter_last;
...
always @(posedge rd_clk_1) begin
    if ((converter_converter_source_valid & converter_converter_source_ready)) begin
        if (converter_converter_last) begin
            converter_converter_mux <= 1'd0;
        end else begin
            converter_converter_mux <= (converter_converter_mux + 1'd1);
        end
    end
    if (rd_rst_1) begin
        converter_converter_mux <= 1'd0;
...
//------------------------------------------------------------------------------
// Memory storage_1: 4-words x 130-bit
//------------------------------------------------------------------------------
// Port 0 | Read: Sync  | Write: Sync | Mode: Read-First  | Write-Granularity: 130 
// Port 1 | Read: Sync  | Write: ---- | 
reg [129:0] storage_1[0:3];
reg [129:0] storage_1_dat0;
reg [129:0] storage_1_dat1;
always @(posedge from133_clk) begin
...
// -----------------------------------------------------------------------------
//  Auto-Generated by LiteX on 2024-11-07 10:12:05.
//------------------------------------------------------------------------------
